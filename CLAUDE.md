...
- `TURSO_DATABASE_URL` / `TURSO_AUTH_TOKEN` — `GET /api/turso` サンプル用の
  Turso (libSQL) 接続。未設定なら 503 を返すのみ。詳細と `@remix-run/data-table`
  との相性は `reference/server/lib/turso/README.md`。

## Turso (libSQL) サンプル

`GET /api/turso` が `@libsql/client` で Turso にアクセスし、`visits` を記録して
累計を返す。Remix の SQL ライブラリ (`@remix-run/data-table` +
`data-table-sqlite`) は**同期** SQLite クライアントを要求するため、非同期の
リモート Turso クライアントとは直接組み合わせられない（詳細・回避策は
