...
- `IDP_ORIGIN` — 外部 IdP の origin (例: `http://localhost:8000`)。
  `/signin` ページが `${IDP_ORIGIN}/authorize` へ DPoP thumbprint と
  redirect_uri を付けて遷移し、戻った後 `${IDP_ORIGIN}/session` で userId を取得する。
