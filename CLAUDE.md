...
`loadConfig(env)` が env レコード (Cloudflare Workers が `fetch(request, env)`
で渡す形) から型付き `Config` を作る。Deno では `getConfig()` が `Deno.env` から
その形を合成する (`Deno.env` へのアクセスはここ 1 箇所)。Cloudflare Workers では
`fetch` ハンドラで `configureFromEnv(env)` を呼べば同じ `getConfig()`
で参照できる。
